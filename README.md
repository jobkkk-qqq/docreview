...
启动后自动打开浏览器访问 `http://127.0.0.1:3000`（前端服务），后端 API 仍运行在 `http://127.0.0.1:8000`
...
A: 等待 3-5 秒，后端启动完成后刷新页面。
   命令行窗口会显示启动日志，看到 `Uvicorn running on http://127.0.0.1:8000` 即表示启动成功。

**Q: 端口 8000 被占用？**
A: 修改 `main_win.py` 中的 `port=8000` 为其他端口。
